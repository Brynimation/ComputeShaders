...
public class GPURenderManager : MonoBehaviour
{
    ...
    public ComputeBuffer _VisiblePositions;
    public ComputeBuffer _FrustumBuffer;

    uint[] indirectArgs = new uint[5] { 0, 0, 0, 0, 0 };
    public int instanceCount;
    public const int POSITION_BUFFER_STRIDE = sizeof(int) * 4;
    ...
    void Start()
    {
        ...
        for (int i = 0; i < instanceCount; i++) 
        {
            ...
            positions[i] = new Vector4(Random.Range(0, 100), Random.Range(0, 100), Random.Range(0, 100), 1f);
        }
    ...
    void Update()
    {
        ...
        {
            Vector4[] posns = new Vector4[instanceCount];
            _Positions.GetData(posns);
            foreach (Vector4 pos in posns)
            {
                Debug.Log(pos);
            }
        ...
        {
            Vector4[] visib = new Vector4[instanceCount];
            _VisiblePositions.GetData(visib);
            foreach (Vector4 pos in visib)
            {
                Debug.Log(pos);
            }
            Debug.Log(visib.Length);
