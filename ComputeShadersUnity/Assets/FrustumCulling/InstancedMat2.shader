Shader "Custom/InstancedMat2"
{
        ...
        SubShader
    {
        ...
        Pass
        {
            ...
            StructuredBuffer<InstanceData> _Positions;
            StructuredBuffer<uint> _Results;
            float _MaxStarSize;
            float4x4 _Matrix;
            ...
            {
                Interpolators o;
                InstanceData data = _Positions[i.index];
                uint result = _Results[data.instanceId];
                _Matrix = CreateMatrix(data.position, float3(1.0, 1.0, 1.0), float3(0.0, 1.0, 0.0), i.index);
